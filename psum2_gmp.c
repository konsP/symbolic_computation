/* 
   ...
   Compile: mpicc -UDEBUG -DCHECK -UPRINT -DDO_FACT -lm -lgmp -O2 -o psum2_gmp psum2_gmp.c
   Run:     mpirun -n 6 psum2_gmp 1M 12

   Batch job of measurements + visualisation:
    > for ((i=2;i<8;i++)) ; do j=$[ $i - 1 ] ; echo "Workers: $j">>L0; mpirun -n ${i} psum2_gmp 400k 12 1>>L0 2>&1 ; done
...
static inline void prods(mpq_t res, ui m, ui n, ui stride);
...
// as above, but in strides
// res = m+(m+i)+...+n
static inline
void sums(mpq_t res, ui m, ui n, ui stride){
  ui i;
  mpz_t p, t;
...
// res = m*(m+1)*...*n
static inline
void prods(mpq_t res, ui m, ui n, ui stride){
  ui i;
  mpz_t p, t;
...
static inline
ui naive_fact_ui(ui n) { return (n==1) ? 1ul : n*naive_fact_ui(n-1); }
...
int main (int argc, char **argv) {
  ...
  if (id == 0) { /*  master   --------------------------------- */
    ...
    if (argc<1) {
      fprintf(stderr, "Usage: psum_gmp <n> ... computes sum over 1 .. n ");
      exit(1);
    }
    ...
    mpq_t result, res, tmp_res, resq_check;
    mpz_t resz_check;
    mpf_t resf;
    ...
    workers=(long)p-1;
#if defined(DEBUG)
    fprintf(stderr, "[%d] computing factorial in strides of %ul\n", workers);
#endif

    /* Distribute work: send intervals to all workers */
    for (i=1; i<p; i++) { 
      // to = (i<=r) ? from+z : from+z-1;
      ...
      MPI_Send(&workers, 1, MPI_LONG, i, 0, MPI_COMM_WORLD);
    }

#if defined(DEBUG)
    fprintf(stderr, "[%d] Master now waiting for results from %ul workers ...\n", id, workers);
#endif
    ...
    /* Collect result: receive string-encoded GMP values from all workers */
...
#  endif
	...
	printf("** Result WRONG\n");
      ...
      mpq_init(resq_check);
      mpq_set_str(resq_check, STR_FACT_50, 10);
#  if defined(PRINT)
      gmp_fprintf(stderr, "expected result of factorial %ul = %Qd\n", n, resq_check);
#  endif
      if (mpq_cmp(result, resq_check)==0) {
      ...
      mpq_init(resq_check);
      mpq_set_str(resq_check, STR_FACT_1000, 10);
#  if defined(PRINT)
      gmp_fprintf(stderr, "expected result of factorial %ul = %Qd\n", n, resq_check);
#  endif
      if (mpq_cmp(result, resq_check)==0) {
...
# endif
    ...
    */
    ...
    MPI_Recv(&stride, 1, MPI_LONG, 0, 0, MPI_COMM_WORLD, &status);

    // start the timer
    elapsed_time = - MPI_Wtime();

    fprintf(stderr, "[%d] from=%ul, to=%ul, stride=%ul\t", id, from, to, stride);
#if defined(DEBUG)
    // fprintf(stderr, "[%d] computing sum ...\n", id);
#endif
    /* Calculate own result */
#if defined(DO_FACT)
    prods(res, from, to, stride);           // result of our interval
#else
    sums(res, from, to, stride);            // result of our interval
#endif
    sz = getSize(res);
