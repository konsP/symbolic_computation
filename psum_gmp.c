/* 
   ...
   Compile: mpicc -UDEBUG -DCHECK -UPRINT -DDO_FACT -lm -lgmp -O2 -o psum_gmp psum_gmp.c
   Run:     mpirun -n 6 psum_gmp 1M 12

   Batch job of measurements + visualisation:
    > for ((i=2;i<8;i++)) ; do j=$[ $i - 1 ] ; echo "Workers: $j">>L0; mpirun -n ${i} psum_gmp 400k 12 1>>L0 2>&1 ; done
...
#if defined(DEBUG)
    fprintf(stderr, "[%d] Master now waiting for results from %d workers ...\n", id, p-1);
#endif
...
#  endif
      ...
      printf("\nWARNING: Check of result is disabled for input larger than 50\n");
    }
# else
